...
func GetHelmMajorVersion(podName, podNamespace, image string) (int, error) {
	...
	var stdout string
	_, stderr, err := kclient.ExecInPod("helm --help", hipconsts.ContainerName, podName, podNamespace,
	...
	stdout, stderr, err = kclient.ExecInPod("helm version --template '{{ $.Version }}'", hipconsts.ContainerName, podName, podNamespace)
	if err != nil {
		return 0, fmt.Errorf("failed to get helm version: %v, stderr: %s", err, stderr)
	}
